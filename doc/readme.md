...
Since the ETSI GS QKD 004 is stateful, it can maintain its path. But maybe an event such as balancing requires an update of the established path. The error-free process works as follows and is displayed in a sequence diagram thereafter (error handling not described here):
