...
The following section states some common sequences for different use-cases and how this API is supposed to support the process. Some aspects, which are beyond the scope are shortened.
...
As an application either opens a keystream via the ETSI GS QKD 004 `open_connect` or requests keys via ETSI GS QKD 014 `enc_keys`, the KMS first has to know via which path through the network the key should be established. The error-free process works as follows and is displayed in a sequence diagram thereafter (error handling not described here):
...
- If the request was an `open_connect` via the ETSI GS QKD 004 interface, it is answered (msg 12) and as soon as the destination application requests the same keystream to be opened (msg 13, 14) the KMS notifies the SDN Agent of this link update (msg 15).
...
**Node 3:**
The KMS should request form the SDN paths which can also support any internal key consumption.

## 2.2. Link update (ETSI 004 use-case)

Since the ETSI GS QKD 004 is stateful it can keep its path. But maybe an event such as balancing requires an update of the established path. The error-free process works as follows and is displayed in a sequence diagram thereafter (error handling not described here):
...
This message can also be used to tear down a link entirely by sending a `DELETE` to `/link/relay/{key_stream_id}` to every node, for example in case of an SDN Controller side initiated close of a key stream.
